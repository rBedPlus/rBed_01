\begin{table*}[t!]  % table* is REQUIRED for full 2-column textwidth
%\begin{table*}[h!]  % table* is REQUIRED for full 2-column textwidth
  % requires package booktabs 
  \vspace*{-6ex}
  \centering
  \small{
  \hspace*{-0em}
  \begin{tabular}{lccccccc} %7 columns, centered,  
    \toprule
    instance & nCols & mRows & maxCD & BKV & UB & value\_Chvatal\_stats  & BKV\_ratio\_stats\\
    \midrule
    %
    {\bf steiner3} &  &  &  &  &  & &\\ 
%steiner3\_009\_12.cnfU & 9 & 12 & 4 & 5 & 10.42 & 5,5,5,0,5 & 1.00,1.00,1.00,0.00,1.00 \\ 
% steiner3\_015\_35.cnfU & 15 & 35 & 7 & 9 & 23.34 & 9,9,9,0,9 & 1.00,1.00,1.00,0.00,1.00 \\ 
 steiner3\_027\_117.cnfU & 27 & 117 & 13 & 18 & 57.24 & 19,19,19,0,19 & 1.06,1.06,1.06,0.00,1.06 \\ 
 steiner3\_045\_330.cnfU & 45 & 330 & 22 & 30 & 110.72 & 31,32,31.87,0.9,33 & 1.03,1.07,1.06,0.03,1.10 \\ 
 steiner3\_081\_1080.cnfU & 81 & 1080 & 40 & 61 & 260.99 & 65,65,65,0,65 & 1.07,1.07,1.07,0.00,1.07 \\ 
 steiner3\_135\_3015.cnfU & 135 & 3015 & 67 & 103 & 493.3 & 107,107,107.92,1.35,111 & 1.04,1.04,1.05,0.01,1.08 \\ 
 steiner3\_243\_9801.cnfU & 243 & 9801 & 121 & 198 & 1064.67 & 211,211,211,0,211 & 1.07,1.07,1.07,0.00,1.07 \\ 
 steiner3\_405\_27270.cnfU & 405 & 27270 & 202 & 335 & 1972.47 & 349,350,350.75,2.02,357 & 1.04,1.04,1.05,0.01,1.07 \\ 
 steiner3\_729\_88452.cnfU & 729 & 88452 & 364 & 617 & 3995.53 & 665,665,665,0,665 & 1.08,1.08,1.08,0.00,1.08 \\ 

\midrule

{\bf orlib}     &      &     &    &    &       &                    &\\ 
 $**$scpb1.cnfU & 3000 & 300 & 29 & 22 & 87.16 & 22,24,23.93,0.5,25 & 1.00,1.09,1.09,0.02,1.14 \\ 
 $**$scpc1.cnfU & 4000 & 400 & 21 & 44 & 160.4 & 44,47,46.86,0.82,50 & 1.00,1.07,1.06,0.02,1.14 \\ 
 $**$scpd1.cnfU & 4000 & 400 & 39 & 25 & 106.34 & 25,27,26.67,0.48,28 & 1.00,1.08,1.07,0.02,1.12 \\ 
 $**$scpb1.cnfW & 3000 & 300 & 29 & 69 & 273.35 & 72,76,75.73,2.06,85 & 1.04,1.10,1.10,0.03,1.23 \\ 
 $**$scpc1.cnfW & 4000 & 400 & 21 & 227 & 827.5 & 249,257,256.67,2.83,265 & 1.10,1.13,1.13,0.01,1.17 \\ 
 $**$scpd1.cnfW & 4000 & 400 & 39 & 60 & 255.21 & 66,71,70.9,1.66,78 & 1.10,1.18,1.18,0.03,1.30 \\
% scpe1.cnfU & 500 & 50 & 18 & 5 & 17.48 & 5,6,5.51,0.5,6 & 1,1.2,1.1,0.1,1.2 \\
 scp41.cnfW & 1000 & 200 & 11 & 429 & 1295.53 & 461,463,466.67,5.12,473 & 1.07,1.08,1.09,0.01,1.10 \\ 
 scp42.cnfW & 1000 & 200 & 10 & 512 & 1499.63 & 568,580,582.68,10.01,612 & 1.11,1.13,1.14,0.02,1.20 \\ 
 scp43.cnfW & 1000 & 200 & 11 & 516 & 1558.26 & 589,591,592.85,3.62,598 & 1.14,1.15,1.15,0.01,1.16 \\ 
 scp44.cnfW & 1000 & 200 & 10 & 494 & 1446.91 & 540,547,547.89,4.24,555 & 1.09,1.11,1.11,0.01,1.12 \\ 
 scp45.cnfW & 1000 & 200 & 11 & 512 & 1546.18 & 571,571,573.91,3,577 & 1.12,1.12,1.12,0.01,1.13 \\ 
 scp46.cnfW & 1000 & 200 & 10 & 560 & 1640.22 & 603,611,611.39,5.16,620 & 1.08,1.09,1.09,0.01,1.11 \\ 
 scp47.cnfW & 1000 & 200 & 12 & 430 & 1334.38 & 474,476,475.01,1,476 & 1.10,1.11,1.10,0.00,1.11 \\ 
 scp48.cnfW & 1000 & 200 & 10 & 492 & 1441.05 & 521,538,538.6,8.65,557 & 1.06,1.09,1.09,0.02,1.13 \\ 
 scp49.cnfW & 1000 & 200 & 11 & 641 & 1935.74 & 741,747,745.51,3.44,750 & 1.16,1.17,1.16,0.01,1.17 \\ 
 scp51.cnfW & 2000 & 200 & 10 & 253 & 741.03 & 282,291,290.45,2.25,295 & 1.11,1.15,1.15,0.01,1.17 \\ 
 scp61.cnfW & 1000 & 200 & 20 & 138 & 496.49 & 152,157,157.08,1.95,163 & 1.10,1.14,1.14,0.01,1.18 \\ 
 scpa1.cnfW & 3000 & 300 & 17 & 253 & 870.21 & 273,286,285.67,4.72,296 & 1.08,1.13,1.13,0.02,1.17 \\ 
% scpb1.cnfW & 3000 & 300 & 29 & 69 & 273.35 & 72,76,75.75,2.12,84 & 1.04,1.1,1.1,0.03,1.22 \\ 
% scpc1.cnfW & 4000 & 400 & 21 & 227 & 827.5 & 249,257,256.83,2.8,264 & 1.1,1.13,1.13,0.01,1.16 \\ 
% scpd1.cnfW & 4000 & 400 & 39 & 60 & 255.21 & 66,71,70.86,1.65,76 & 1.1,1.18,1.18,0.03,1.27 \\ 

 \midrule
 {\bf random} &  &  &  &  & & \\ 
 m100\_050\_10\_10.cnfU & 50 & 100 & 31 & 8 & 32.22 & 8,8,8.36,0.48,9 & 1.00,1.00,1.04,0.06,1.12 \\ 
 m100\_100\_10\_10.cnfU & 100 & 100 & 17 & 12 & 41.27 & 13,14,13.6,0.51,15 & 1.08,1.17,1.13,0.04,1.25 \\ 
 m100\_100\_10\_15.cnfU & 100 & 100 & 22 & 10 & 36.91 & 10,11,11.21,0.47,13 & 1.00,1.10,1.12,0.05,1.30 \\ 
 m100\_100\_10\_30.cnfU & 100 & 100 & 32 & 9 & 36.53 & 9,10,9.92,0.83,12 & 1.00,1.11,1.10,0.09,1.33 \\ 
 m100\_100\_30\_30.cnfU & 100 & 100 & 43 & 6 & 26.1 & 6,6,6,0,6 & 1.00,1.00,1.00,0.00,1.00 \\ 
 m200\_100\_10\_30.cnfU & 100 & 200 & 51 & 11 & 49.71 & 11,12,11.99,0.29,13 & 1.00,1.09,1.09,0.03,1.18 \\ 
 m200\_100\_30\_50.cnfU & 100 & 200 & 99 & 6 & 31.06 & 6,6,6,0,6 & 1.00,1.00,1.00,0.00,1.00 \\
              
  \midrule
 {\bf tiny} &  &  &  &  &  &\\ 
% affil\_9\_12.cnfU & 9 & 12 & 4 & 5 & 10.42 & 5,5,5,0,5 & 1,1,1,0,1 \\ 
 chvatal\_6\_5.cnfW & 6 & 5 & 5 & 1.1 & 2.51 & 2.28,2.28,2.28,0,2.28 & 2.07,2.07,2.07,0.00,2.07 \\ 
% cormen\_a\_6\_12.cnfU & 6 & 12 & 6 & 3 & 7.35 & 4,4,4,0,4 & 1.33,1.33,1.33,0,1.33 \\ 
% cormen\_b\_12\_6.cnfU & 12 & 6 & 3 & 3 & 5.5 & 3,3,3,0,3 & 1,1,1,0,1 \\ 
% ruler\_9\_22.cnfU & 9 & 22 & 7 & 5 & 12.96 & 5,5,5,0,5 & 1,1,1,0,1 \\ 
% ruler\_9\_22.cnfW & 9 & 22 & 7 & 8.5 & 22.04 & 10,10,10,0,10 & 1.18,1.18,1.18,0,1.18 \\ 
% school\_11\_12.cnfU & 11 & 12 & 7 & 3 & 7.78 & 3,3,3,0,3 & 1,1,1,0,1 \\ 
% school\_11\_12.cnfW & 11 & 12 & 7 & 6 & 15.56 & 6.5,6.5,6.5,0,6.5 & 1.08,1.08,1.08,0,1.08 \\ 
% school\_13\_14.cnfU & 13 & 14 & 6 & 4 & 9.8 & 4,4,4,0,4 & 1,1,1,0,1 \\ 
% school\_13\_14.cnfW & 13 & 14 & 6 & 8 & 19.6 & 8,9.5,9.48,1.05,11.5 & 1,1.19,1.19,0.13,1.44 \\ 
% school\_15\_16.cnfU & 15 & 16 & 6 & 5 & 12.25 & 5,5,5,0,5 & 1,1,1,0,1 \\ 
% school\_15\_16.cnfW & 15 & 16 & 6 & 9.5 & 23.28 & 9.5,11,11.12,0.97,13 & 1,1.16,1.17,0.1,1.37 \\ 
% school\_17\_18.cnfU & 17 & 18 & 6 & 6 & 14.7 & 6,6,6,0,6 & 1,1,1,0,1 \\ 
% school\_17\_18.cnfW & 17 & 18 & 6 & 10.5 & 25.73 & 10.5,12,12.22,0.85,14 & 1,1.14,1.16,0.08,1.33 \\ 
% school\_19\_20\_a.cnfW & 19 & 20 & 6 & 3.833 & 9.39 & 4.17,4.5,4.56,0.25,5 & 1.09,1.17,1.19,0.06,1.3 \\ 
% chosol\_5\_6.cnfU & 5 & 6 & 4 & 3 & 6.25 & 3,3,3,0,3 & 1,1,1,0,1 \\ 
% school\_5\_6.cnfW & 5 & 6 & 4 & 5 & 10.42 & 5,5,5,0,5 & 1,1,1,0,1 \\ 
% school\_6\_7.cnfU & 6 & 7 & 4 & 3 & 6.25 & 3,3,3,0,3 & 1,1,1,0,1 \\ 
% school\_7\_8.cnfU & 7 & 8 & 4 & 3 & 6.25 & 3,3,3.34,0.48,4 & 1,1,1.11,0.16,1.33 \\ 
% school\_8\_10.cnfU & 8 & 10 & 3 & 4 & 7.33 & 4,5,4.64,0.67,6 & 1,1.25,1.16,0.17,1.5 \\ 
 school\_9\_11\_\_0.cnfU & 9 & 11 & 3 & 4 & 7.33 & 4,5,4.85,0.68,6 & 1.00,1.25,1.21,0.17,1.50 \\ 
% school\_9\_11\_\_0.cnfW & 9 & 11 & 3 & 14.25 & ???? &  14.25,14.25,14.25,0,14.25 & 1.00,1.00,1.00,0.00,1.00 \\ 
 school\_9\_16.cnfU & 9 & 16 & 6 & 5 & 12.25 & 6,6,6,0,6 & 1.20,1.20,1.20,0.00,1.20 \\ 
 school\_9\_16.cnfW & 9 & 16 & 6 & 10 & 24.5 & 10,10.5,10.57,0.54,11.5 & 1.00,1.05,1.06,0.05,1.15 \\ 
% school\_19\_20.cnfU & 19 & 20 & 6 & 6 & 14.7 & 6,6,6,0,6 & 1,1,1,0,1 \\ 
 school\_19\_20.cnfW & 19 & 20 & 6 & 11.5 & 28.18 & 11.5,13.5,13.39,0.91,15.5 & 1.00,1.17,1.16,0.08,1.35 \\ 
% stack\_5\_6.cnfU & 5 & 6 & 4 & 2 & 4.17 & 3,3,3,0,3 & 1.5,1.5,1.5,0,1.5 \\ 
% stack\_5\_6.cnfW & 5 & 6 & 4 & 2 & 4.17 & 2,2,2,0,2 & 1,1,1,0,1 \\    
    \midrule
    instance & nCols & mRows & maxCD & BKV & UB & value\_Chvatal\_stats  & BKV\_ratio\_stats\\                   
    \bottomrule
  \end{tabular}
  }
  
\caption{
This table relates to both Section~\ref{sec_matching} and Section~\ref{sec_cover}.
Columns denote the name of the instance file, 
number of instance columns ({\it nCols}), 
number of instance rows ({\it mRows}), 
maximum degree column  ({\it maxCD}), 
best-known-value of the minimum set cover ({\it BKV}), 
Chvatal's upper bound on the minimum set cover ({\it UB}), 
observed set cover statistics with the Chvatal's greedy algorithm ({\it value\_Chvatal\_stats}), and
set cover statistics normalized with {\it BKV}, ({\it BKV\_ratio\_stats}).
The reported statistics represents values of {\it minimum, median, mean, standard deviation, and maximum}.
Instance file name extensions, {\it .cnfU} and {\it .cnfW}, denote instances with unit weighted columns
and pre-assigned weighted columns, respectively. 
The first six instances denoted with $**$ under {\sf orlib} have special significance, see 
Section~\ref{sec_cover} for details.
}
  
%\OMIT{ 
%\caption{
%Statistical experiments with 1000 replications and chvatal\_S solver with data libraries, short names in terms of steiner3, orlib, random, and tiny. Instance names start with ** are experiments with 10,000 replications. The table contains instance name {\it instance}, max size of column degrees {\it maxColDeg}, best known value {\it valueTarget}, upper bound for the instance {\it upperBound}, statistical information for values returned from Chvatal greedy algorithm {\it valueGreedy\_stats}, statistical information for ratio of values from greedy algorithm dividing by best know value {\it ratio\_stats}. All statistical information include min, median, mean, standard deviation, and max in this order separating by comma. The Chvatal greedy algorithm at most cases is efficient when dealing with the instance files in this table, only when reading steiner3\_729\_88452.cnfU, the statistical information of runtime performance is 6.35,8.49,8.83,1.24,14.52 in seconds.
%}
%}
  \label{tb_bgmc_data}
  
\end{table*}