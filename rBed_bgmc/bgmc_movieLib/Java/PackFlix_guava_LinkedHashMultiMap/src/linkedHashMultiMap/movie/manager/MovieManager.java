package linkedHashMultiMap.movie.manager;
...
import linkedHashMultiMap.dsa.list.*;
import linkedHashMultiMap.dsa.sorter.Sorter;
import linkedHashMultiMap.movie.data.Movie;
import linkedHashMultiMap.movie.data.WatchRecord;
import linkedHashMultiMap.movie.factory.*;
import linkedHashMultiMap.movie.io.*;
...
public class MovieManager {
	...
	public List<Movie> getMostFrequentlyWatchedMovies(int numberOfMovies) {
		...
		LinkedHashMultimap<String, Integer> wmap = DSAFactory.getMap();
		for (int i = 0; i < watchlist.size(); i++) {
...
// 			}
	...
	public List<Movie> getMoviesByWatchDuration(int threshold) {
		...
		LinkedHashMultimap<String, Integer> map = DSAFactory.getMap();
		for (int i = 0; i < watchlist.size(); i++) {
			WatchRecord r = watchlist.get(i);
			String id = r.getMovieId();
			...
			if (count != 0) {
				Set<Integer> val_list = map.get(id);
				int val = 0;
				Iterator<Integer> it = val_list.iterator();
				while (it.hasNext()) {
		...
		for (int i = 0; i < movielist.size(); i++) {
			...
			if (count != 0) {
				Set<Integer> val_list = map.get(movie.getId());
				int val = 0;
				Iterator<Integer> it = val_list.iterator();
				while (it.hasNext()) {
					int val_new = it.next();
