package hashMultiMap.movie.manager;
...
import hashMultiMap.dsa.list.*;
import hashMultiMap.dsa.sorter.Sorter;
import hashMultiMap.movie.data.Movie;
import hashMultiMap.movie.data.WatchRecord;
import hashMultiMap.movie.factory.*;
import hashMultiMap.movie.io.*;
...
public class MovieManager {
	...
	public List<Movie> getMostFrequentlyWatchedMovies(int numberOfMovies) {
		...
		HashMultimap<String, Integer> wmap = DSAFactory.getMap();
		for (int i = 0; i < watchlist.size(); i++) {
...
// 			}
	...
	public List<Movie> getMoviesByWatchDuration(int threshold) {
		...
		HashMultimap<String, Integer> map = DSAFactory.getMap();
		for (int i = 0; i < watchlist.size(); i++) {
			WatchRecord r = watchlist.get(i);
			String id = r.getMovieId();
